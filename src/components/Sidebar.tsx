...
import { Plus, LayoutTemplate, Clock, Home, Bot, Sparkles, FolderKanban, X } from "lucide-react";
...
export function Sidebar({ 
  ...
  return (
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-zinc-950 border-r border-zinc-800 flex flex-col h-screen shrink-0 text-zinc-300 transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0`}>
        <div className="p-5 border-b border-zinc-800/50 flex items-center justify-between">
          <button 
            onClick={() => { onNewProject(); onClose(); }}
            className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white font-medium py-2.5 px-4 rounded-xl transition-all shadow-sm flex items-center justify-center gap-2 group"
          >
              ...
              {projects.length === 0 ? (
                ...
                projects.map(project => (
                  ...
                  >
                    ...
                    <span className="truncate">{project.title || project.idea.substring(0, 28) + "..."}</span>
                  </button>
                ))
              )}
            </div>
